...
def etl(**context):
    ...
    # 임시 테이블 생성
    create_sql = f"""BEGIN;DROP TABLE IF EXISTS {schema}.temp_{table};
    CREATE TABLE {schema}.temp_{table} AS SELECT * FROM {schema}.{table};"""
    ...
    # 기존 테이블 대체
    alter_sql = f"""BEGIN;DELETE FROM {schema}.{table};
